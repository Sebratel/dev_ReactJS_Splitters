...
function normalizeStreetForRelief(street) {
...
export async function querySplitterNeighborsWithOrigin(pool, code, radiusMeters) {
  /**
   * Coordenadas só em `authentication_splitters` (sem JOIN caixa de rede): evita 500 por divergência de schema
   * (`network_boxes` / colunas). O mapa no detalhe usa COALESCE(caixa, splitter) vindo da SPLITTERS_BASE_QUERY.
   */
...
export async function evaluateReliefForSplitter(pool, code, opts) {
  ...
  const targetStreet = normalizeStreetForRelief(targetRowRes.rows?.[0]?.street);
  ...
  for (let i = 0; i < capped.length; i++) {
    ...
    if (rm == null) continue;
    const neighborStreet = normalizeStreetForRelief(capped[i].street);
    const sameStreet =
      targetStreet !== null &&
      neighborStreet !== null &&
