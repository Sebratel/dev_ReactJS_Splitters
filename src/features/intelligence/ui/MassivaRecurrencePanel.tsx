...
function usageCellClass(usagePercent: number): string {
  if (usagePercent >= 95) return 'text-rose-700 font-semibold'
  if (usagePercent >= 70) return 'text-amber-800 font-semibold'
...
export function MassivaRecurrencePanel({
  ...
  return (
    <article className="rounded-3xl border border-white/50 bg-white/70 p-4 shadow-xl shadow-amber-500/10 backdrop-blur-xl">
      ...
      <div className="mt-4">
        ...
        {insights.ranking.length === 0 ? (
          <p className="mt-3 rounded-xl border border-dashed border-slate-200 bg-slate-50/80 px-3 py-6 text-center text-sm text-slate-500">
            Nenhum equipamento com massiva registrada no histórico local neste período.
          </p>
        ) : (
          <div className="mt-2 overflow-auto rounded-xl border border-slate-200/80">
            <table className="w-full min-w-[720px] text-left text-xs">
              <thead className="border-b border-slate-200/80 bg-slate-50/90 text-[10px] uppercase tracking-wide text-slate-500">
                <tr>
              ...
              <tbody className="divide-y divide-slate-100">
                {insights.ranking.map((row, index) => (
                  <tr key={row.splitterCode} className="hover:bg-amber-50/40">
                    ...
                    <td className="px-2 py-2">
                      ...
                      <span className="font-mono text-[10px] text-slate-500">{row.splitterCode}</span>
                    </td>
                    ...
                    <td className="max-w-[140px] truncate px-2 py-2 text-slate-600" title={row.oltLabel}>
                      {row.oltLabel}
                    </td>
                    <td className="max-w-[160px] truncate px-2 py-2 text-slate-600" title={row.locationLabel}>
                      {row.locationLabel}
                    </td>
                  </tr>
                ))}
      ...
      <div className="mt-4">
        <button
          ...
          className="flex w-full items-center justify-between gap-2 rounded-xl border border-slate-200/80 bg-slate-50/80 px-3 py-2 text-left text-xs font-semibold text-slate-700 hover:bg-slate-100/80"
        >
        ...
        {histogramOpen ? (
          <div className="mt-2 space-y-2 rounded-xl border border-slate-200/80 bg-white px-3 py-3">
      ...
      {insights.showBarChart ? (
                  ...
                  content={({ active, payload }) => {
                    ...
                    return (
                      <div className="rounded-lg border border-slate-200 bg-white px-2.5 py-2 text-xs shadow-lg">
                        <p className="font-bold text-slate-900">{d.splitterTitle}</p>
                        <p className="font-mono text-[10px] text-slate-500">{d.splitterCode}</p>
        ...
        <p className="mt-4 rounded-xl border border-amber-200/80 bg-amber-50/60 px-3 py-2.5 text-[11px] leading-relaxed text-amber-950">
...
function KpiCard({
  ...
  const valueClass =
    tone === 'amber' ? 'text-amber-950' : tone === 'violet' ? 'text-violet-950' : 'text-slate-900'
