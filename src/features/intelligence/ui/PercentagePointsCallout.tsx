...
export function PercentagePointsCallout({ periodLabel, className }: PercentagePointsCalloutProps) {
  ...
  return (
    <div
      className={cn(
        'flex gap-2 rounded-xl border border-sky-200/80 bg-sky-50/70 px-2.5 py-2 text-[11px] leading-relaxed text-slate-700',
        className,
      )}
      title={PP_TOOLTIP}
    >
      <CircleHelp size={14} className="mt-0.5 shrink-0 text-sky-700" aria-hidden />
      <p>
        ...
        <span className="font-semibold text-slate-900">70% → 78% = +8 pp</span> (não é “+11% em cima de 70%”).
