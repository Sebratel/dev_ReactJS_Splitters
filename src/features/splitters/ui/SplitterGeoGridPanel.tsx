...
export function SplitterGeoGridPanel({ rows }: SplitterGeoGridPanelProps) {
  return (
    <section
      className="rounded-xl border border-neutral-200 dark:border-white/10 bg-surface-container-lowest p-5 dark:border-neutral-700 dark:bg-neutral-900"
      aria-labelledby="splitter-geogrid-heading"
    >
      <h2
        id="splitter-geogrid-heading"
        className="text-base font-semibold text-on-surface dark:text-neutral-100"
      >
        GeoGrid - portas e reservas
      </h2>
      <p className="mt-1 text-sm text-on-surface-variant dark:text-on-surface-variant/60">
      ...
      <div className="mt-4 overflow-x-auto">
        <table className="w-full min-w-[32rem] border-collapse text-left text-sm">
          <thead>
            <tr className="border-b border-neutral-200 dark:border-white/10 text-on-surface-variant dark:border-neutral-600 dark:text-on-surface-variant/60">
          ...
          <tbody>
            {rows.map((row) => (
              ...
              >
                ...
                <td className="py-2 pr-3 text-on-surface dark:text-neutral-200">
                  {formatReservaLabel(row)}
                </td>
                <td className="py-2 pr-3 text-on-surface dark:text-neutral-200">
                  {row.clienteNome ?? (row.idCliente ? `ID ${row.idCliente}` : '-')}
                </td>
                <td className="py-2 text-on-surface-variant dark:text-on-surface-variant/60">
